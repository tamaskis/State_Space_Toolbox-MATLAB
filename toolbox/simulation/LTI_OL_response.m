%==========================================================================
%
% LTI_OL_response  Dynamic response of a linear time-invariant system
% given the open loop matrices.
%
%   [t,x] = LTI_OL_response(A)
%   [t,x,~,u] = LTI_OL_response(A,B)
%   [t,x,y,u] = LTI_OL_response(A,B,C,D)
%   [t,x,y,u] = LTI_OL_response(A,B,C,D,u)
%   [__] = LTI_OL_response(__,opts)
%
% Author: Tamas Kis
...
% INPUT:
% ------
%   A       - (n×n double) dynamics matrix
%   B       - (OPTIONAL) (n×m double) input matrix
%   C       - (OPTIONAL) (p×n double) output matrix
%   D       - (OPTIONAL) (p×m double) feedforward matrix
...
%
%==========================================================================
function [t,x,y,u] = LTI_OL_response(A,B,C,D,u,opts)

    % --------------------------------------------------
    % Determines if optional parameters have been input.
    % --------------------------------------------------

    B_input = (nargin >= 2) && ~isempty(B);
    C_input = (nargin >= 3) && ~isempty(C);
    D_input = (nargin >= 4) && ~isempty(D);
    u_input = (nargin >= 5) && ~isempty(u);
    opts_input = (nargin == 6) && ~isempty(opts);
    ...
    n = size(A,1);
    if B_input, m = size(B,2); end
    if C_input, p = size(C,1); end
    ...
    % defaults D to 0 if B and C are input but D is not
    if B_input && C_input && ~D_input
        D = zeros(p,m);
    end
    ...
    D_exists = exist('D','var');
    if C_input
        y = zeros(N+1,p);
        if D_exists && u_exists
            for i = 1:(N+1)
                y(i,:) = (C*x(i,:)'+D*u(i,:)')';
            end
        else
